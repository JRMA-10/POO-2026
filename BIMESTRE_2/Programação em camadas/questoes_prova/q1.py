...
class Cliente:
    ...
    def set_nascimento(self, nasc): 
        if nasc > datetime.now(): raise ValueError('A data precisa ser válida')
    ...
    def __str__(self):
        return f"{self.__id} - {self.__nome} - {self.__email} - {self.__fone} - {self.get_nascimento().strftime('%d/%m/%Y')}"
    
    def to_json(self):
        return { "id":self.__id, "nome":self.__nome, "email":self.__email,
    "fone":self.__fone, "nascimento" : self.__nascimento.strftime('%d/%m/%Y') }
